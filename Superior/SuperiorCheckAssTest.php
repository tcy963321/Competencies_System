...
<div class="w3-container">
 ...
 <?php
   ...
   <tr>	
	...
	<td><?php echo "<form action='SuperiorAssignAssTestDetails.php?TestID=$row[TestID]&EmployeeID=$row[EmployeeID]' method='POST'><button type='submit' name='Edit' title='View Question'><i class='fas fa-eye'></i></i></button></form><form class='delete_button' action='Superiorprocesscompetency.php?TestID=$row[TestID]& EmployeeID=$row[EmployeeID]' method='POST'><button type='submit' title='Delete' name='AssignUserTestDel'><i class='fas fa-trash-alt'></i></button></form>";?></td>
	</tr>
  <?php	} } ?>	 
</table></center>
