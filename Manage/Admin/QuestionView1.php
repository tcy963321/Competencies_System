...
<?php
                      $result = ViewQuestionEvaDetails();
                      if($result){
                      while ($row = mysqli_fetch_assoc($result)){?>   
<tr>
	<th>Competency Code</th>
	<td><?php echo $row['CodeACompetency']; ?>&nbsp;&nbsp;&nbsp; <?php echo "<form action='QuestionView1.php?EQCODE=$row[A_QCode]&CodeE=$row[CodeACompetency]' method='POST'><button type='submit' name='Edit' title='View Competency' class='collapsible'> More>> </button></form>";?>
	<div class="contents">
 <?php
                      $results = CheckCompetencyEvaDetails();
                      if($results){
