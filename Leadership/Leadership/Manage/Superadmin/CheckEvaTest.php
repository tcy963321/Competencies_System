...
<div class="w3-container">
 ...
 <?php
   ...
   <tr>	
	...
	<td><?php echo "<form action='AssignEvaTestDetails1.php?TestID=$row[TestID]&EmployeeID=$row[EmployeeID]' method='POST'><button type='submit' name='Edit' title='View Question'><i class='fas fa-eye'></i></i></button></form><form class='delete_button' action='CheckEvaTest.php?TestID=$row[TestID]& EmployeeID=$row[EmployeeID]' method='POST'><button type='submit' title='Delete' name='EvaluatorTestDel'><i class='fas fa-trash-alt'></i></button></form>";?></td>
	</tr>
  <?php	} } ?>	 
</table></center>
