...
$sql = "SELECT account.EmployeeID,account.Name,account.IC,account.Contact_Number,account.Gender,account.Date_of_Birth,account.Address,account.Email,account.Department,account.Position,account.image_url,login.Password,login.Roles,login.Status FROM account JOIN login ON account.EmployeeID=login.username where Roles='Admin' order by EmployeeID";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
	$imageURL = 'imagesuploadadmin/'.$row["image_url"];	
echo "<tr><td>" . $row["EmployeeID"]. "</td><td>" . $row["Name"] . "</td><td>". $row["IC"]. "</td><td>" . $row["Contact_Number"]. "</td><td>" . $row["Gender"] . "</td><td>". $row["Date_of_Birth"]. "</td><td>" . $row["Address"]. "</td><td>" . $row["Email"] . "</td><td>". $row["Department"]. "</td><td>" . $row["Position"]. "</td><td>" . $row["Roles"] . "</td><td>". $row["Status"]. "</td><td><form action='Superviewadmindetails.php?id=$row[EmployeeID]' method='POST'><input type='submit' name='Details' value='Details' /></form><form action='SuperEditAdmin.php?id=$row[EmployeeID]' method='POST'><input type='submit' name='Edit' value='Edit' /></form><form class='delete_button' action='SuperadminDeleteRoles.php?id=$row[EmployeeID]' method='POST'><input type='submit' name='Delete' value='Delete' /></form></td></tr>";
