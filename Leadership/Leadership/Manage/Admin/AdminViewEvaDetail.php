...
<?php
                      $results=ViewUserEvaDetails();
                      if($results){
                      $rows=mysqli_fetch_array($results);	{?> 
 <fieldset>
  <legend>Evaluate Details:</legend>
			<table>
				<tr>
				<td>Candidate :</td>
...
<?php
                      $result = ViewHistoryEvaDetails();
                      if($result){
                      while ($row = mysqli_fetch_assoc($result)){?>   
<tr>
	<td><?php echo $row['E_QCode']; ?></td>
	<td><?php echo $row['VoteE'];?></td>
	<td><?php echo "<form action='QuestionView1.php?EQCODE=$row[E_QCode]' method='POST' target='_blank' rel='noopener noreferrer'><button type='submit' name='Edit' title='View Question'><i class='fas fa-eye'></i></i></button></form>";?></td>
</tr>
  <?php	} } ?>						  
</table></center>
