...
<div class="tp-content">
	...
	<div class="strip-profile">View User</div>
...
$sqls=mysqli_query($conn,"SELECT * FROM account where EmployeeID='$ID' ");
$rows  = mysqli_fetch_array($sqls);	
$department=$rows['Department'];
$position=$rows['Position'];

$sql = "SELECT account.EmployeeID,account.Name,account.IC,account.Contact_Number,account.Gender,account.Date_of_Birth,account.Address,account.Email,account.Department,account.Position,account.image_url,login.Password,login.Roles,login.Status FROM account JOIN login ON account.EmployeeID=login.Username WHERE account.Department='$department' AND login.Roles='User'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
echo "<tr><td>" . $row["EmployeeID"]. "</td><td>" . $row["Name"] . "</td><td>". $row["IC"]. "</td><td>" . $row["Contact_Number"]. "</td><td>" . $row["Gender"] . "</td><td>". $row["Date_of_Birth"]. "</td><td>" . $row["Address"]. "</td><td>" . $row["Email"] . "</td><td>". $row["Department"]. "</td><td>" . $row["Position"]. "</td><td>" . $row["Roles"] . "</td><td>". $row["Status"]. "</td><td><form class='Details_button' action='SuperiorDeteailsuser.php?id=$row[EmployeeID]' method='POST'><input type='submit' name='Deteails' value='Deteails' /></form><form action='SuperiorEditUser.php?id=$row[EmployeeID]' method='POST'><input type='submit' name='Edit' value='Edit' /></form><form class='delete_button' action='SuperiorDeleteRoles.php?id=$row[EmployeeID]' method='POST'><input type='submit' name='Delete' value='Delete' /></form></td></tr>";
